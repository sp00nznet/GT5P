...
# Appldr keys, parsed from data/keys (scetool format), keyed by
# (self_type, key_revision) -> (erk, riv). NPDRM and APP sets are separate.
def load_appldr_keys(path=None):
    path = Path(path or Path(__file__).parent.parent / "data" / "keys")
    out, cur = {}, None
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if line.startswith("["):
            ...
            cur[k.strip()] = v.strip()
            if k.strip() == "ctype" and cur.get("section") == "appldr":
                try:
                    rev = int(cur.get("revision", "0"), 16)
                    erk = bytes.fromhex(cur["erk"])
                    riv = bytes.fromhex(cur["riv"])
                except (KeyError, ValueError):
                    continue
                if len(erk) == 32 and len(riv) == 16:
                    out.setdefault((cur.get("self_type", "APP"), rev), (erk, riv))
    return out


APPLDR_KEYS = load_appldr_keys()
...
def rap_to_klicensee(rap_bytes):
    """Convert a 16-byte RAP into the content klicensee (5 rounds of the
    published pbox/e1/e2 mixing, matching scetool's implementation)."""
    ...
    for _ in range(5):
        ...
        for i in range(15, 0, -1):
            key[RAP_PBOX[i]] ^= key[RAP_PBOX[i - 1]]
...
def decrypt_self(self_data, klicensee=None):
    ...
    key_rev = sf.key_type
    self_type = "NPDRM" if content_key else "APP"
    print(f"  Key revision: 0x{key_rev:04X}  self_type={self_type}")

    # NPDRM layer: the 0x40-byte encryption root header is AES-128-CBC decrypted
    # with the klicensee-derived key and a zero IV before the appldr pass. (The
    # loader does not XOR the two keys together.)
    if content_key:
        enc_root = aes_cbc_dec(content_key, bytes(16), enc_root)

    matched = ([(f"{self_type} rev 0x{key_rev:04X}", APPLDR_KEYS[(self_type, key_rev)])]
               if (self_type, key_rev) in APPLDR_KEYS else [])
    others = [(f"fallback {t} rev 0x{r:04X}", v)
              for (t, r), v in APPLDR_KEYS.items() if (t, r) != (self_type, key_rev)]

    for label, (erk, riv) in matched + others:
        dec_root = aes_cbc_dec(erk, riv, enc_root)
        mk, mk_pad, mi, mi_pad = dec_root[0:16], dec_root[16:32], dec_root[32:48], dec_root[48:64]
        pad_zeros = sum(1 for b in mk_pad + mi_pad if b == 0)
        if pad_zeros >= 30:
            metadata_key, metadata_iv = mk, mi
            print(f"  Key set {label}: pad_zeros={pad_zeros}/32 -> correct")
            break
...
def main():
    ...
    output = Path(sys.argv[2]) if len(sys.argv) > 2 else input_dir / (eboot.stem + ".ELF")

    # Find RAP
    rap_path = None
    if len(sys.argv) > 3:
        rap_path = Path(sys.argv[3])
    else:
        for d in (eboot.parent, eboot.parent.parent, input_dir):
            raps = sorted(d.glob("*.rap"))
            if raps:
                rap_path = raps[0]
                break
